...
# =====================================================================
# ÁLGEBRA LINEAL - MOTOR MÓDULO A (100% IDÉNTICO AL ORIGINAL)
# =====================================================================
def transpose_matrix_a(M):
...
def invert_lower_triangular_a(L):
    n = len(L)
...
def invert_matrix_nxn_a(M):
    if not M or not M[0]: return None
    try:
        L = cholesky_decompose_a(M)
        L_inv = invert_lower_triangular_a(L)
        return matmul_a(transpose_matrix_a(L_inv), L_inv)
    except:
        return gauss_jordan_inverse_a(M)

# =====================================================================
# ÁLGEBRA LINEAL - MOTOR MÓDULO B (INDEPENDIENTE Y ROBUSTO)
# =====================================================================
def transpose_matrix_b(M):
    if not M or not M[0]: return []
    try: return [[M[j][i] for j in range(len(M))] for i in range(len(M[0]))]
    except IndexError: return []
...
def invert_lower_triangular_b(L):
    n = len(L)
    inv = [[0.0] * n for _ in range(n)]
    for i in range(n):
        inv[i][i] = 1.0 / L[i][i]
...
def invert_matrix_nxn_b(M):
    if not M or not M[0]: return None
    try:
        L = cholesky_decompose_b(M)
        L_inv = invert_lower_triangular_b(L)
        return matmul_b(transpose_matrix_b(L_inv), L_inv)
    except:
        return gauss_jordan_inverse_b(M)

# =====================================================================
# PARSER RINEX - MÓDULO A (100% CÓDIGO ORIGINAL PURO SIN ALTERACIONES)
# =====================================================================
def parse_rinex_obs_completo_a(path):
    obs = {}
    sys_idx = {}
    sys_tokens = {}
    last_sys_char = None
...
# =====================================================================
# PARSER RINEX - MÓDULO B (FLEXIBLE PARA DIFERENTES MARCAS / NOMENCLATURAS)
# =====================================================================
def parse_rinex_obs_completo_b(path):
    obs = {}
    sys_idx = {}
    sys_tokens = {}
    last_sys_char = None
        ...
        for line in f:
            if in_h:
                if "SYS / # / OBS TYPES" in line:
                    ...
                    for sc, t in sys_tokens.items():
                        sys_idx[sc] = {
                            'C1': next((i for i, x in enumerate(t) if x.startswith('C1') or x.startswith('C2') or x.startswith('C')), -1),
                            'L1': next((i for i, x in enumerate(t) if x.startswith('L1') or x.startswith('L2') or x.startswith('L')), -1),
                            'C5': next((i for i, x in enumerate(t) if x.startswith('C5') or x.startswith('C7') or x.startswith('C8')), -1),
                            'L5': next((i for i, x in enumerate(t) if x.startswith('L5') or x.startswith('L7') or x.startswith('L8')), -1),
                            'S1': next((i for i, x in enumerate(t) if x.startswith('S1') or x.startswith('S2') or x.startswith('S')), -1),
                            'S5': next((i for i, x in enumerate(t) if x.startswith('S5') or x.startswith('S7') or x.startswith('S8')), -1)
                        }
            elif line.startswith('>'):
                p = line[1:].split()
                if len(p) >= 6:
                ...
                if idx_s1 >= 0 and len(line) >= 17 + 16 * idx_s1:
                    v = line[3+16*idx_s1 : 17+16*idx_s1].strip()
                    if v: data['S1'] = float(v.replace('S1', '30').replace('D', 'E').replace('d', 'e'))
                if idx_s5 >= 0 and len(line) >= 17 + 16 * idx_s5:
                    v = line[3+16*idx_s5 : 17+16*idx_s5].strip()
                    if v: data['S5'] = float(v.replace('S5', '30').replace('D', 'E').replace('d', 'e'))
                
                valid_p = ('C1' in data and data['C1'] > 10000000.0) or ('C5' in data and data['C5'] > 10000000.0)
                if valid_p:
                    obs.setdefault(tow, {})[line[0:3].strip()] = data
    return obs
# =====================================================================
# SINCRONIZACIÓN Y RINEX - MÓDULO A (PURO)
# =====================================================================
def interpolar_base_a_rover_a(obs_base, tr, max_gap=0.05):
...
def generar_rinex_sincronizado_a(raw_path, out_path, obs_dict):
    header_lines = []
...
# =====================================================================
# SINCRONIZACIÓN Y RINEX - MÓDULO B (TOLERANTE A HETEROGENEIDAD)
# =====================================================================
def interpolar_base_a_rover_b(obs_base, tr, max_gap=0.25):
    tiempos_base = sorted(list(obs_base.keys()))
    if not tiempos_base: return None
    idx = min(range(len(tiempos_base)), key=lambda i: abs(tiempos_base[i] - tr))
    if abs(tiempos_base[idx] - tr) <= max_gap: return obs_base[tiempos_base[idx]].copy()
...
def generar_rinex_sincronizado_b(raw_path, out_path, obs_dict):
    header_lines = []
    with open(raw_path, 'r', encoding='utf-8', errors='ignore') as f:
        for line in f:
            if "SYS / # / OBS TYPES" in line: continue 
...
# =====================================================================
# PRODUCTOS IGS Y EFEMÉRIDES (COMPARTIDO Y ROBUSTO)
# =====================================================================
SP3_CACHE = {}
SP3_CACHE_KEYS = []
MAX_CACHE_SIZE = 2048
...
# =====================================================================
# MOTOR PPK INDEPENDIENTE - MÓDULO A (100% CÓDIGO ORIGINAL PURO)
# =====================================================================
def aislar_diferencias_simples_ppk_a(obs_b, obs_r):
    sd_suavizada = {}
...
# =====================================================================
# MOTOR PPK INDEPENDIENTE - MÓDULO B (CON COMPENSACIÓN ISB / DCB)
# =====================================================================
def aislar_diferencias_simples_ppk_b(obs_b, obs_r):
    sd_suavizada = {}
    for tow in sorted(list(obs_r.keys())):
        if tow not in obs_b: continue
        ...
        for s, d_r in obs_r[tow].items():
            ...
            delta_isb = 0.0
            if s[0] == 'G': delta_isb = 1.25 if wave_sys == WAVE_L5 else 0.85
            elif s[0] == 'E': delta_isb = 1.05 if wave_sys == WAVE_L5 else 0.65
            else: delta_isb = 0.95
            
            sd_epoca[s] = {
                'sd_P': (pr_r - pr_b) - delta_isb, 
                'pr_b': pr_b, 'pr_r': pr_r, 
                'cp_b': cp_b, 'cp_r': cp_r,
                'wave': wave_sys,
                'snr': min(snr_b, snr_r),
...
# =====================================================================
# LAMBDA Z-TRANSFORM Y SUAVIZADOR RTS DUAL
# =====================================================================
def decorrelacion_lambda_z_a(Q):
    n = len(Q)
    Z = matid_a(n)
    try:
        L = cholesky_decompose_a(Q)
    except:
        return Z, Q 
    ...
    Z_T = transpose_matrix_a(Z)
    Q_z = matmul_a(matmul_a(Z_T, Q), Z)
    return Z, Q_z

def decorrelacion_lambda_z_b(Q):
    n = len(Q)
    Z = matid_b(n)
    try:
        L = cholesky_decompose_b(Q)
    except:
        return Z, Q 
    for i in range(n - 1, -1, -1):
        for j in range(i - 1, -1, -1):
            ...
            if mu != 0:
                for k in range(j + 1): L[i][k] -= mu * L[j][k]
                for k in range(n): Z[k][i] -= mu * Z[k][j]
    Z_T = transpose_matrix_b(Z)
    Q_z = matmul_b(matmul_b(Z_T, Q), Z)
    return Z, Q_z

def suavizador_rts_backward_a(forward_states):
...
def suavizador_rts_backward_b(forward_states):
    n = len(forward_states)
    if n == 0: return []
    ...
    for k in range(n-2, -1, -1):
        ...
        P_pri_inv = invert_matrix_nxn_b(P_pri_k1)
        if not P_pri_inv:
            smoothed_states[k] = forward_states[k]['X_post']
            continue
            
        C_k = matmul_b(P_post_k, P_pri_inv)
        
        X_smooth_k1 = smoothed_states[k+1]
        X_pri_k1 = forward_states[k+1]['X_pri']
        
        dx = [[X_smooth_k1[i][0] - X_pri_k1[i][0]] for i in range(3)]
        correction = matmul_b(C_k, dx)
...
# =====================================================================
# FILTRO KALMAN EKF - MÓDULO A (100% CÓDIGO ORIGINAL PURO)
# =====================================================================
def procesar_ekF_lambda_a(sd_epoca, nav, sp3, kf_estado, tr, mask_angle, snr_mask):
    try:
        ...
        for s in sat_list:
            ...
            if data['cp_r'] is not None and data['cp_b'] is not None and rc['cp_r'] is not None and rc['cp_b'] is not None:
                ...
                if cp_valid:
                    ...
                    Z_trans, Q_z = decorrelacion_lambda_z_a(var_amb)
        ...
        H_T = transpose_matrix_a(H)
        R_inv = matid_a(len(R_diag))
        for i in range(len(R_diag)): R_inv[i][i] = 1.0 / max(1e-6, R_diag[i])
        
        P_inv = invert_matrix_nxn_a(P_pri)
        if not P_inv: return None, "FAILED", kf_estado, None
        
        H_T_R_inv = matmul_a(H_T, R_inv)
        N_mat = matadd_a(matmul_a(H_T_R_inv, H), P_inv)
        U_vec = matmul_a(H_T_R_inv, L)
        
        Q_cov = invert_matrix_nxn_a(N_mat)
        if not Q_cov: return None, "FAILED", kf_estado, None
        
        Delta_X = matmul_a(Q_cov, U_vec)
...
# =====================================================================
# FILTRO KALMAN EKF - MÓDULO B (TOLERANTE Y COMPENSADO ISB)
# =====================================================================
def procesar_ekF_lambda_b(sd_epoca, nav, sp3, kf_estado, tr, mask_angle, snr_mask):
    try:
        X_pri = [[kf_estado['X'][0][0]], [kf_estado['X'][1][0]], [kf_estado['X'][2][0]]]
        P_pri = [row[:] for row in kf_estado['P']]
        h_r = kf_estado.get('h_r', 0.0)
        ...
        for s, d in sd_epoca.items():
            ...
            if sp_r and sp_b:
                el_r, az_r = calcular_topocentricas(sp_r[0], sp_r[1], sp_r[2], X_apc, Y_apc, Z_apc)
                if el_r >= mask_angle and d.get('snr', 25.0) >= snr_mask:
                    sat_positions[s] = {'sp_r': sp_r, 'sp_b': sp_b, 'sd_P': d['sd_P'], 'cp_r': d['cp_r'], 'cp_b': d['cp_b'], 'wave': d['wave'], 'snr': d['snr'], 'sys': d['sys']}
        ...
        for s in sat_list:
            ...
            var_base = (10.0 ** (-data['snr'] / 10.0)) * 100.0 * 2.0
            ...
            if data['cp_r'] is not None and data['cp_b'] is not None and rc['cp_r'] is not None and rc['cp_b'] is not None:
                ...
                if s in kf_estado['prev_cp']:
                    delta_P = data['sd_P'] - kf_estado['prev_cp'][s]['P']
                    delta_CP = (data['cp_r'] - data['cp_b']) - kf_estado['prev_cp'][s]['CP']
                    if abs(delta_P - delta_CP * wave) > 8.0: 
                        cp_valid = False
                ...
                if cp_valid:
                    ...
                    var_amb = [[var_base * 0.0001]]
                    Z_trans, Q_z = decorrelacion_lambda_z_b(var_amb)
                    ...
                    if abs(ambiguity_float - amb_restored) < 0.35:
                        L.append([(DD_CP_m - amb_restored * wave) - DD_CP_calc])
                        H.append(dx_geom)
                        R_diag.append(var_base * 0.0001)
                        kf_estado['fix_flags'] += 1
        ...
        H_T = transpose_matrix_b(H)
        R_inv = matid_b(len(R_diag))
        for i in range(len(R_diag)): R_inv[i][i] = 1.0 / max(1e-6, R_diag[i])
        
        P_inv = invert_matrix_nxn_b(P_pri)
        if not P_inv: return None, "FAILED", kf_estado, None
        
        H_T_R_inv = matmul_b(H_T, R_inv)
        N_mat = matadd_b(matmul_b(H_T_R_inv, H), P_inv)
        U_vec = matmul_b(H_T_R_inv, L)
        
        Q_cov = invert_matrix_nxn_b(N_mat)
        if not Q_cov: return None, "FAILED", kf_estado, None
        
        Delta_X = matmul_b(Q_cov, U_vec)
        ...
        status = "FIXED (PPK)" if kf_estado['fix_flags'] > 3 else "FLOAT (DGPS)"
        kf_estado['fix_flags'] = 0 
        ...
        return None, f"FAILED_EXCEPTION:_{str(e)}", kf_estado, None
# =====================================================================
# ESTADÍSTICAS Y FILTRADO VINCULANTE (CON RESCATE ADAPTATIVO)
# =====================================================================
...
# =====================================================================
def generar_informe_homogeneizacion_detallado(base_name, rover_name, base_raw, rover_raw, rover_sinc, modo_modulo='A'):
    def get_stats(obs):
    ...
    desc_mod = "Módulo A (Misma Marca - 100% Original Puro)" if modo_modulo == 'A' else "Módulo B (Diferentes Marcas - ISB/DCB)"
    
    informe = f"""
========================================================================
    AUDITORÍA FORENSE DE EMPAREJAMIENTO DE ÉPOCAS [{desc_mod}]
========================================================================
...
def generar_informe_ascii(tipo, p_dict):
    modo_ej = p_dict.get('modo_modulo', 'A')
    ...
    desc_mod_text = "Módulo A (Misma Marca - Original)" if modo_ej == 'A' else "Módulo B (Diferentes Marcas)"
    
    informe = f"""
========================================================================
             INFORME DE PROCESAMIENTO GNSSJP PRO [{desc_mod_text}]
========================================================================
...
------------------------------------------------------------------------
  [-] Módulo Operativo       : {desc_mod_text}
...
# =====================================================================
# RUTA FLASK: INTERFAZ HTML PRINCIPAL CON SELECTOR DE MÓDULO A / B
# =====================================================================
@app.route('/')
def index():
...
<head>
    <meta charset="UTF-8">
    <title>GNSSJP PRO - Sistema Híbrido Dual (Módulo A / Módulo B)</title>
    <style>
        body { font-family: monospace; background: #0f172a; color: #38bdf8; padding: 20px; }
        .container { max-width: 900px; margin: auto; background: #1e293b; padding: 25px; border-radius: 8px; border: 1px solid #334155; }
        h1 { color: #f8fafc; font-size: 20px; text-align: center; border-bottom: 2px solid #38bdf8; padding-bottom: 10px; }
        .module-selector { background: #0f172a; padding: 15px; border: 2px dashed #38bdf8; margin-bottom: 20px; border-radius: 6px; }
        label { display: block; margin-top: 10px; color: #cbd5e1; }
        input, select { width: 100%; padding: 8px; margin-top: 5px; background: #0f172a; border: 1px solid #475569; color: #f8fafc; border-radius: 4px; box-sizing: border-box; }
        button { background: #0284c7; color: white; border: none; padding: 10px 15px; margin-top: 15px; cursor: pointer; border-radius: 4px; font-weight: bold; width: 100%; }
        button:hover { background: #0ea5e9; }
        pre { background: #0f172a; padding: 15px; border: 1px solid #334155; color: #4ade80; overflow-x: auto; white-space: pre-wrap; margin-top: 20px; border-radius: 4px; }
    </style>
</head>
<body>
    <div class="container">
        <h1>GNSSJP PRO - HÍBRIDO DETERMINISTA DUAL (MÓDULO A / B)</h1>
        
        <div class="module-selector">
            <label for="modo_modulo"><strong>SELECCIÓN DE MOTOR DE CÁLCULO FORENSE:</strong></label>
            <select id="modo_modulo" name="modo_modulo">
                <option value="A">Módulo A: Misma Marca (100% Original Puro)</option>
                <option value="B">Módulo B: Diferentes Marcas (Compensación ISB / DCB)</option>
            </select>
        </div>

        <form id="form_proceso" onsubmit="event.preventDefault(); ejecutarProceso();">
            <label>URL Google Drive Base RINEX (.obs):</label>
            <input type="text" id="url_base" required placeholder="https://drive.google.com/file/d/...">
            
            <label>URL Google Drive Rover Calibración (.obs):</label>
            <input type="text" id="url_rover" required placeholder="https://drive.google.com/file/d/...">
            
            <div style="display: flex; gap: 10px;">
                <div style="flex:1;"><label>UTM Norte Base (m):</label><input type="number" id="utm_norte" step="0.001" value="0.0"></div>
                <div style="flex:1;"><label>UTM Este Base (m):</label><input type="number" id="utm_este" step="0.001" value="0.0"></div>
                <div style="flex:1;"><label>Cota Base (m):</label><input type="number" id="utm_cota" step="0.001" value="0.0"></div>
            </div>
            
            <div style="display: flex; gap: 10px;">
                <div style="flex:1;"><label>UTM Norte Rover Real (m):</label><input type="number" id="utm_norte_r" step="0.001" value="0.0"></div>
                <div style="flex:1;"><label>UTM Este Rover Real (m):</label><input type="number" id="utm_este_r" step="0.001" value="0.0"></div>
                <div style="flex:1;"><label>Cota Rover Real (m):</label><input type="number" id="utm_cota_r" step="0.001" value="0.0"></div>
            </div>

            <button type="submit">EJECUTAR CALIBRACIÓN Y ANÁLISIS HÍBRIDO</button>
        </form>

        <pre id="console_output">Esperando ejecución de tareas...</pre>
    </div>

    <script>
        async function ejecutarProceso() {
            const out = document.getElementById('console_output');
            out.textContent = "> [SISTEMA] Iniciando flujo secuencial completo...";
            
            const formData = new FormData();
            formData.append('url_base', document.getElementById('url_base').value);
            formData.append('url_rover', document.getElementById('url_rover').value);
            formData.append('utm_norte', document.getElementById('utm_norte').value);
            formData.append('utm_este', document.getElementById('utm_este').value);
            formData.append('utm_cota', document.getElementById('utm_cota').value);
            formData.append('utm_norte_r', document.getElementById('utm_norte_r').value);
            formData.append('utm_este_r', document.getElementById('utm_este_r').value);
            formData.append('utm_cota_r', document.getElementById('utm_cota_r').value);
            formData.append('modo_modulo', document.getElementById('modo_modulo').value);

            try {
                let res = await fetch('/API/tab1_homogenizar', { method: 'POST', body: formData });
                let text = await res.text();
                out.textContent = text;
                
                if(!text.includes("[SUCCESS]")) { out.textContent += "\\n> [ERROR] Detenido en Etapa 1."; return; }

                out.textContent += "\\n> [SISTEMA] Descargando Efemérides NAV...";
                res = await fetch('/API/tab2_efemerides', { method: 'POST' });
                text = await res.text();
                out.textContent += "\\n" + text;

                if(!text.includes("[SUCCESS]")) { out.textContent += "\\n> [ERROR] Detenido en Etapa 2."; return; }

                out.textContent += "\\n> [SISTEMA] Ejecutando Malla EKF...";
                let formCalib = new FormData();
                formCalib.append('param_max_gap', '0.5');
                formCalib.append('param_snr', '25.0');
                formCalib.append('param_iter', '6');
                formCalib.append('modo_modulo', document.getElementById('modo_modulo').value);
                
                res = await fetch('/API/tab3_calibrar', { method: 'POST', body: formCalib });
                text = await res.text();
                out.textContent += "\\n" + text;
    ...
    return html_content
# =====================================================================
# RUTAS FLASK - SISTEMA HÍBRIDO DUAL INDEPENDIENTE (MÓDULO A / B)
# =====================================================================
...
def tab1_homogenizar():
    ...
    modo_modulo = request.form.get('modo_modulo', 'A')
    ...
    guardar_estado('modo_modulo', modo_modulo)
    ...
    def procesar():
        try:
            ...
            yield f"\n> [SISTEMA] Iniciando Etapa 1: Emparejamiento (Módulo {'A - Original Puro' if modo_modulo=='A' else 'B - Compensado ISB/DCB'})...\n"
            
            # Bifurcación estricta de parser independiente
            if modo_modulo == 'A':
                base_raw_dict = parse_rinex_obs_completo_a(p_b_raw)
                rover_raw_dict = parse_rinex_obs_completo_a(p_r_raw)
            else:
                base_raw_dict = parse_rinex_obs_completo_b(p_b_raw)
                rover_raw_dict = parse_rinex_obs_completo_b(p_r_raw)
            ...
            for tr in sorted(list(rover_raw_dict.keys())):
                ...
                if modo_modulo == 'A':
                    base_interp = interpolar_base_a_rover_a(base_raw_dict, tr, max_gap=float('inf'))
                else:
                    base_interp = interpolar_base_a_rover_b(base_raw_dict, tr, max_gap=float('inf'))
            ...
            if modo_modulo == 'A':
                generar_rinex_sincronizado_a(p_b_raw, p_b_h, base_sinc)
                generar_rinex_sincronizado_a(p_r_raw, p_r_h, rover_sinc)
            else:
                generar_rinex_sincronizado_b(p_b_raw, p_b_h, base_sinc)
                generar_rinex_sincronizado_b(p_r_raw, p_r_h, rover_sinc)
            ...
            yield generar_informe_homogeneizacion_detallado(name_base, name_rover, base_raw_dict, rover_raw_dict, rover_sinc, modo_modulo)
            yield "\n[SUCCESS]"
    ...
    def procesar():
        try:
            ...
            if not os.path.exists(nav_path):
                ...
                if not descargado: raise Exception("HTTP 404: Ningún servidor IGS ha publicado aún el archivo NAV.")
...
def tab3_calibrar():
    ...
    modo_modulo = request.form.get('modo_modulo') or leer_estado('modo_modulo') or 'A'
    ...
    def procesar():
        try:
            yield f"> [SISTEMA] Iniciando Búsqueda Determinista (Módulo {modo_modulo} | {p_iter} Iteraciones)...\n"
            ...
            if not nav_path or not p_b_h or not p_r_h: 
                yield "> [ERROR FATAL] Faltan archivos en memoria.\n"; return

            # --- BIFURCACIÓN ESTRICTA DE PARSERS Y MOTORES DE CÁLCULO ---
            if modo_modulo == 'A':
                obs_b_raw = parse_rinex_obs_completo_a(p_b_h)
                obs_r_raw = parse_rinex_obs_completo_a(p_r_h)
                sd_suavizada = aislar_diferencias_simples_ppk_a(obs_b_raw, obs_r_raw)
            else:
                obs_b_raw = parse_rinex_obs_completo_b(p_b_h)
                obs_r_raw = parse_rinex_obs_completo_b(p_r_h)
                sd_suavizada = aislar_diferencias_simples_ppk_b(obs_b_raw, obs_r_raw)
            ...
            P_init = matid_a(3) if modo_modulo == 'A' else matid_b(3)
            ...
            for t in t_sample:
                if modo_modulo == 'A':
                    sem, status, kf_estado_raw, _ = procesar_ekF_lambda_a(sd_suavizada[t], parse_rinex_nav_real(nav_path), parse_sp3_preciso(sp3_path) if sp3_path else {}, kf_estado_raw, t, 10.0, p_snr)
                else:
                    sem, status, kf_estado_raw, _ = procesar_ekF_lambda_b(sd_suavizada[t], parse_rinex_nav_real(nav_path), parse_sp3_preciso(sp3_path) if sp3_path else {}, kf_estado_raw, t, 10.0, p_snr)
            ...
            if modo_modulo == 'A':
                obs_b_full = parse_rinex_obs_completo_a(p_b_raw) if p_b_raw and os.path.exists(p_b_raw) else obs_b_raw
                obs_r_full = parse_rinex_obs_completo_a(p_r_raw) if os.path.exists(p_r_raw) else obs_r_raw
            else:
                obs_b_full = parse_rinex_obs_completo_b(p_b_raw) if p_b_raw and os.path.exists(p_b_raw) else obs_b_raw
                obs_r_full = parse_rinex_obs_completo_b(p_r_raw) if os.path.exists(p_r_raw) else obs_r_raw
                
            rover_tows_full = sorted(list(obs_r_full.keys()))
            base_tows_full = sorted(list(obs_b_full.keys()))
            nav_data = parse_rinex_nav_real(nav_path)
            sp3_data = parse_sp3_preciso(sp3_path) if sp3_path else {}
            ...
            for nivel in range(p_iter):
                ...
                for gap in set(gap_grid):
                    ...
                    if modo_modulo == 'A':
                        sd_suav = aislar_diferencias_simples_ppk_a(obs_b_sync, obs_r_full)
                    else:
                        sd_suav = aislar_diferencias_simples_ppk_b(obs_b_sync, obs_r_full)
                    ...
                    for m in set(m_grid):
                        for snr in set(snr_grid):
                            ...
                            for t in t_samp:
                                if modo_modulo == 'A':
                                    sem, status, kf_est, _ = procesar_ekF_lambda_a(sd_suav[t], nav_data, sp3_data, kf_est, t, m, snr)
                                else:
                                    sem, status, kf_est, _ = procesar_ekF_lambda_b(sd_suav[t], nav_data, sp3_data, kf_est, t, m, snr)
                            ...
                            for cp in set(cp_grid):
                                for ca in set(ca_grid):
                                    ...
                                    if ret < max(10, int(len(coords) * 0.04)): continue
            ...
            if global_best_score != float('inf'):
                ...
                yield "\n========================================================\n"
                yield f"   [INFORME] PARÁMETROS ÓPTIMOS GLOBALES [MÓDULO {modo_modulo}]\n"
                yield "========================================================\n"
...
def tab4_procesar():
    ...
    modo_modulo = leer_estado('modo_modulo') or 'A'

    url_rover_nuevo = request.form.get('url_rover_nuevo')
    
    if p_mask is None or utm_n is None:
        return Response("> [ERROR FATAL] Parámetros no encontrados. Ejecute la Pestaña 3.\n", mimetype='text/plain')

    if not url_rover_nuevo or url_rover_nuevo.strip() == '': 
        return Response("> [ERROR] Falta URL del nuevo RINEX Rover.\n", mimetype='text/plain')
    ...
    def procesar():
        try:
            ...
            yield f"\n> [SISTEMA] Procesando (EKF + RTS Independiente | Módulo {modo_modulo})...\n"
            nav_path = leer_estado('nav_path')
            sp3_path = leer_estado('sp3_path')
            p_b_raw = leer_estado('base_raw') 

            if not nav_path or not p_b_raw or not os.path.exists(p_b_raw): 
                yield "> [ERROR FATAL] Faltan archivos base.\n"; return

            if modo_modulo == 'A':
                obs_b_raw = parse_rinex_obs_completo_a(p_b_raw)
                obs_r_raw = parse_rinex_obs_completo_a(p_r_nuevo)
                sd_suavizada = aislar_diferencias_simples_ppk_a(obs_b_raw, obs_r_raw)
            else:
                obs_b_raw = parse_rinex_obs_completo_b(p_b_raw)
                obs_r_raw = parse_rinex_obs_completo_b(p_r_nuevo)
                sd_suavizada = aislar_diferencias_simples_ppk_b(obs_b_raw, obs_r_raw)
            ...
            if len(sd_suavizada) == 0: yield "\n> [ERROR] Sin épocas sincronizadas.\n"; return
            ...
            for t in sd_suavizada:
                ...
                if modo_modulo == 'A':
                    sem, status, kf_est, st_dict = procesar_ekF_lambda_a(sd_suavizada[t], nav, sp3, kf_est, t, p_mask, p_snr)
                    sm_states = suavizador_rts_backward_a(fwd_states) if fwd_states else []
                else:
                    sem, status, kf_est, st_dict = procesar_ekF_lambda_b(sd_suavizada[t], nav, sp3, kf_est, t, p_mask, p_snr)
                    sm_states = suavizador_rts_backward_b(fwd_states) if fwd_states else []
            ...
            sm_states = suavizador_rts_backward_a(fwd_states) if modo_modulo == 'A' else suavizador_rts_backward_b(fwd_states)
            coords = []
            ...
            if res_estadistica[0] is None and modo_modulo == 'B':
                yield f"\n> [AVISO] Tolerancia estricta superada. Ampliando límites de tolerancia (x2.5) para rescatar épocas...\n"
                res_estadistica = estadistica_desacoplada(coords, p_cp * 2.5, p_ca * 2.5, err_hor_max * 2.5 if err_hor_max > 0 else 0.0, err_ver_max * 2.5 if err_ver_max > 0 else 0.0)
